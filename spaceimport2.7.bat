...
REM Updated and Tested General and conference imports and are working now
REM changed the connection to the salto server for uploading files a new username and password was created on the server and as a part of setup these Windows credentials need to be added to the credentials manager
REM Added in a check to ensure that the connection to the salto server has been made and prompt to rety if not
REM Added a check for the saved credentials on machine before processing the script.
REM Version 2.7
REM This should be a release candidate
REM =================================NOTES=======================================================

REM ==============================Pending Changes / Improvements=================================
REM 2022-08-08 Better instructions for saving as CSV for users STATUS: Improvements
REM ==============================Pending Changes / Improvements=================================

REM =====================KNOWN ISSUES / BUGS=====================================================
REM YYYY-MM-DD Sample logging of issues. STATUS: (Bug/Improvements/Fixed-Testing Required/Fix-Confirmed)
REM 2022-08-08 Better instructions for saving as CSV for users STATUS: Improvements
REM 2022-08-08 Need a check for the saved credentials for connecting to the server STATUS:Fixed-Confirmed
REM =====================KNOWN ISSUES / BUGS=====================================================



set VERSION=2.7
set COMPILED=August 8th, 2022
...
:CREDENTIALSCHECK
cmdkey /list |findstr "172.25.126.100" > nul
if "%ERRORLEVEL%" == "1" (
	echo looks like you do not have the required credentials saved to allow for this import process to complete. 
	echo Please submit a ticket at sju.ca/rt-it to have IT assist with this.
	PAUSE
	EXIT
)
Exit /b
...
for /F "skip=1 tokens=1-20 delims=," %%a in (%CONFIRMEDFILE%) do (
	echo I have a:%%a,b:%%b,c:%%c
	PAUSE
	...
	SET /A COUNT=COUNT+1
	REM echo the count is now:%COUNT%
	echo 1;%EXTID%!COUNT!;%%b;%%a;!USERTITLE!;0;1;0;%EXPIRY% >> %USERNAME%import%today%.csv
)
...
curl -LJo accesslevelswithids.txt  https://%GITHUBKEY%@github.com/tait-kelly/Salto/raw/main/accesslevelswithids.txt > NUL
for /F "skip=1 tokens=1-20 delims=," %%a in (%CONFIRMEDFILE%) do (
	REM The format for file is Last Name(A),First Name(B),Building (Ryan Hall (North Tower) or Siegfried Hall (South Tower)of FINN)(C),Floor (Level X)(D),Room (XXXX)(E)
	set floor=%%d
	REM echo floor should be the last number of %%f
	REM echo I want to set the floor to:!floor:~-1!
	set floornum=!floor:~-1!
...
:UPLOADCHECK
if NOT EXIST "x:" (
	echo Upload to the salto server failed, you may have entered your credentials wrong or not have permissions.
	set /p RETRY="Do you want to retry?(y/n)"
	if /I "%RETRY%"=="y" call:COPYTOSALTO
	else GOTO EOF
)
exit /b
